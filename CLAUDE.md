...
- **Le tableau 1:1 d'une campagne hérite de la visibilité de la CAMPAGNE** (`campaigns.is_shared`/`created_by`), JAMAIS de son propre `dashboards.is_shared` (toujours false). Enforced dans `loadAccessible` de `/api/dashboards/[id]` ET la visibilité de `.../data` (sinon une campagne partagée renvoie 404 → le builder redirige vers `/dashboards`, « Mes tableaux »). `/data` ne doit PAS se contenter de `campaign_id !== null` (fuite : compteurs d'une campagne privée lisibles par tous). Même invariant dans edh/ganprev. Builder en lecture seule quand `dashboard.can_edit===false`. (migration 013 ; bug corrigé 2026-06-15.)
- **Tests Vitest : ponter `getSupabaseScoped` vers `getSupabase` dans le mock** — `vi.mock("@/lib/supabase/service", () => { const getSupabase = vi.fn(); return { getSupabase, getSupabaseScoped: vi.fn(() => getSupabase()) }; })`. Sinon l'automock renvoie `undefined` et `sb.from` plante → TOUTE la suite de la route échoue en silence. Slug de test = `neoma` ; slug étranger (rejet inter-école) = `other-school`.
